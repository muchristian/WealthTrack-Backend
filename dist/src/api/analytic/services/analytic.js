...
const filterDataByDate = (data, type, date) => {
  ...
  for (const d of data) {
    ...
    const result =
      (0, date_util_1.dateFormatBytype)(
        `${(0, date_fns_1.format)(parsedDt, "dd")}-${(0, date_fns_1.format)(
          parsedDt,
          "MM"
        )}-${(0, date_fns_1.format)(parsedDt, "yyyy")}`,
        type
      ) === date;
    if (result) {
      arr.push(arr, d);
...
exports.default = strapi_1.factories.createCoreService(
  ...
  ({ strapi }) => ({
    async find(functions) {
      ...
      const { actualStartDate, actualEndDate } = (0,
      date_util_1.getActualDateRange)(
        ctx,
        (0, date_util_1.parseDate)(ctx, filter.dateFrom),
        (0, date_util_1.parseDate)(ctx, filter.dateTo)
      );
      const dates = (0, date_fns_1.eachDayOfInterval)({
        ...
        (0, date_util_1.dateFormatBytype)(
          `${(0, date_fns_1.format)(d, "dd")}-${(0, date_fns_1.format)(
            d,
            "MM"
          )}-${(0, date_fns_1.format)(d, "yyyy")}`,
          type
        )
      );
