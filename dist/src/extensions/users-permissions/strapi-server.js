...
function default_1(plugin) {
    ...
    plugin.controllers.auth["register"] = async (ctx) => {
        ...
        return (0, response_1.success)("User authenticated successfully", await (0, sanitize_1.sanitizeOutput)(result, ctx, strapi.getModel("plugin::users-permissions.user")));
    };
    // authentication
    plugin.controllers.auth["callback"] = async (ctx) => {
        ...
        const validPassword = strapi.plugins["users-permissions"].services["user"].validatePassword(password, user.password);
        if (!validPassword) {
            throw new ValidationError("Invalid email or password");
        }
        // if (!user.confirmed) {
        ...
        return (0, response_1.success)("User authenticated successfully", await (0, sanitize_1.sanitizeOutput)(user, ctx, strapi.getModel("plugin::users-permissions.user")));
    };
